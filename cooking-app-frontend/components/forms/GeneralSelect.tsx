...
type Props<T> = {
  ...
  fetchOptions: (id: string, signal?: AbortSignal) => Promise<T[]>;

  placeholder?: string;
  className?: string;
};

export default function GeneralSelect<T>({
  translationsKey = "",
  showShadow = false,
  label,
  idParameter,
  ...
  useEffect(() => {
    ...
    clearedForSelectionRef.current = false;
  }, [value, getName]);
  ...
  async function loadOptions(id: string) {
    ...
    setLoading(true);
    try {
      const items = await fetchOptions(id, controller.signal);
  ...
  useEffect(() => {
    ...
    if ((value && q === getName(value)) || !idParameter) {
      setOptions([]);
      return;
    }

    const timer = setTimeout(() => {
      void loadOptions(idParameter);
    }, 300);

    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [query, value, getName, idParameter]);
